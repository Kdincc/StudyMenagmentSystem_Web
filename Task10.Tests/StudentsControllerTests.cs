    ...
    public class StudentsControllerTests
    {
        ...
        public async Task Edit_Get_OperationCanceledException_RedirectsToIndex()
        {
            ...
            _studentsServiceMock
                .Setup(service => service.GetEditStudentDtoAsync(It.IsAny<int>(), It.IsAny<CancellationToken>()))
                .ThrowsAsync(new OperationCanceledException());

            //Act
            var result = await _controller.Edit(1, new CancellationToken());
            var redirectResult = result as RedirectToActionResult;
        ...
        {
            //Arrange
            var viewModel = new EditStudentViewModel { Id = 1, Name = "John", LastName = "Doe", GroupId = 1 };

            //Setup
            _studentsServiceMock
                .Setup(service => service.EditStudentAsync(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<int>(), It.IsAny<int>(), It.IsAny<CancellationToken>()))
                .ThrowsAsync(new OperationCanceledException());


            //Act
            var result = await _controller.Edit(viewModel, new CancellationToken());
            var redirectResult = result as RedirectToActionResult;
        ...
        public async Task Delete_Get_OperationCanceledException_RedirectsToIndex()
        {
            //Setup  
            _studentsServiceMock
                .Setup(service => service.GetDeleteStudentDto(It.IsAny<int>(), It.IsAny<CancellationToken>()))
                .ThrowsAsync(new OperationCanceledException());

            //Act
            var result = await _controller.Delete(1, new CancellationToken());
            var redirectResult = result as RedirectToActionResult;
        ...
        {
            //Arrange
            var viewModel = new DeleteStudentViewModel { Id = 1 };
                
            //Setup
            _studentsServiceMock
                .Setup(service => service.DeleteStudentAsync(It.IsAny<int>(), It.IsAny<CancellationToken>()))
                .ThrowsAsync(new OperationCanceledException());

            //Act
            var result = await _controller.Delete(viewModel, new CancellationToken());
            var redirectResult = result as RedirectToActionResult;
        ...
        public async Task Create_Get_OperationCanceledException_RedirectsToIndex()
        {
            //Setup
            _studentsServiceMock
                .Setup(service => service.GetGroupsAsync(It.IsAny<CancellationToken>()))
                .ThrowsAsync(new OperationCanceledException());

            //Act
            var result = await _controller.Create(new CancellationToken());
        ...
        {
            //Arrange
            var viewModel = new CreateStudentViewModel { Name = "John", LastName = "Doe", GroupId = 1 };

            //Setup
            _studentsServiceMock
                .Setup(service => service.CreateStudentAsync(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<int>(), It.IsAny<CancellationToken>()))
                .ThrowsAsync(new OperationCanceledException());

            //Act
            var result = await _controller.Create(viewModel, new CancellationToken());
            var redirectResult = result as RedirectToActionResult;
